...
class Romav2Matcher(BaseMatcher):
    def __init__(self, device="cpu", max_num_keypoints=2048, *args, **kwargs):
        ...
        super().__init__(device, **kwargs)
        self.model = RoMaV2().to(self.device)
        self.max_keypoints = max_num_keypoints
    ...
    def preprocess(self, img: torch.Tensor, pad=False) -> Image:
        ...
        #     if (self.model.H_hr is None or self.model.W_hr is None)
        #     else (self.model.H_hr, self.model.W_hr)
        # )
        # _, h, w = img.shape
        # orig_shape = h, w

        # img = tfm.Resize((H, W), antialias=True)(img)
    ...
    def _forward(self, img0, img1, pad=False):
        if pad:
            self.compute_padding(img0, img1)
        img0, img0_size = self.preprocess(img0)
        print(img0)
        img1, img1_size = self.preprocess(img1)
        w0, h0 = img0_size
        w1, h1 = img1_size
        ...
        matches, overlaps, precision_AB, precision_BA = self.model.sample(preds, self.max_keypoints)
