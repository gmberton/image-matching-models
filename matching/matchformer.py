...
class MatchformerMatcher(BaseMatcher):
    weights_src = (
       "https://drive.google.com/file/d/1Ii-z3dwNwGaxoeFVSE44DqHdMhubYbQf/view"
    )
    ...
    def load_model(self, cfg_path=None):
        ...
        matcher.load_state_dict({k.replace('matcher.',''):v  for k,v in torch.load(self.weights_path, map_location='cpu').items()})
