...
def download_example_images(repo_id, output_dir):
    ...
    for example in tqdm(
        dataset["train"], "Downloading example images"
...
def wrap_images(
    ...
    if geo_info is not None and len(geo_info) != 0:
        ...
        return plot_to_array(fig), rectified_image1
    else:
        return None, None


def run_generate_warp_images(
...
def run_ransac(
    ...
    # )
    num_matches_ransac = pred["inlier_kpts0"]
    logger.info(f"Display matches done using: {time.time()-t1:.3f}s")
    t1 = time.time()

    # compute warp images
    output_wrapped = stich(img0=image0, img1=image1)
...
def run_matching(
    ...
    Args:
        ...
        model_name (str): key of the model to use.
    ...
    t0 = time.time()
    matcher = get_matcher(model_name, max_num_keypoints=extract_max_keypoints)

    # update: ransac method
    matcher.ransac_method = ransac_zoo[ransac_method]
    matcher.ransac_reproj_thresh = ransac_reproj_threshold
    matcher.ransac_conf = ransac_confidence
    matcher.ransac_iters = ransac_max_iter

    # TODO: cache model

    # model_cfg = matcher_zoo[model_name]
    # efficiency = model_cfg["info"].get("efficiency", "high")
    ...
    image0_pil = Image.fromarray(image0).convert("RGB")
    image1_pil = Image.fromarray(image1).convert("RGB")

    image0 = tfm.ToTensor()(image0_pil)
    image1 = tfm.ToTensor()(image1_pil)

    # pred keys:
    # num_inliers, H, all_kpts0, all_kpts1, all_desc0, all_desc1, matched_kpts0, matched_kpts1, inlier_kpts0, inlier_kpts1
    pred = matcher(image0, image1)

    logger.info(f"Matching images done using: {time.time()-t1:.3f}s")
    t1 = time.time()

    # plot images with keypoints
    fig_output_keypoints, _ = plot_matches(
        img0=image0,
        img1=image1,
        result=pred,
    ...
    fig_output_matches_raw, _ = plot_matches(
        img0=image0,
        img1=image1,
        result=pred,
        show_matched_kpts=False,
    ...
    # plot images with ransac matches
    fig_output_matches_ransac, _ = plot_matches(
        img0=image0,
        img1=image1,
        result=pred,
    ...
    # plot wrapped images
    output_wrapped, warped_image = wrap_images(
        np.array(image0_pil),
        np.array(image1_pil),
        {"Homography": pred["H"]},
        choice_geometry_type,
