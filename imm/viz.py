...
def plot_kpts(img0, result, model_name="", save_path=None) -> list[matplotlib.axes.Axes]:
    ...
    if model_name is not None:
        viz2d.add_text(0, f"{len(result['all_kpts0'])}", fs=20)
...
def stitch(img0: np.ndarray | torch.Tensor, img1: np.ndarray | torch.Tensor, result: dict) -> np.ndarray:
    ...
    Args:
        ...
        result (dict): BaseMatcher return dict of results. Required keys: ["H"]
    ...
    stitched_imgs = cv2.warpPerspective(img0, H_translation.dot(result["H"]), (x_max - x_min, y_max - y_min))
